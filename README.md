<div align="center">

# Hi, I'm Suhani

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=7C3AED&center=true&vCenter=true&width=500&lines=Full-Stack+Software+Developer;Backend+%26+Real-time+Systems;LeetCode+Knight+%F0%9F%8F%85;Open+to+Work+%F0%9F%9F%A2)](https://git.io/typing-svg)

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://inahus-portfolio.netlify.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/inahus99)
[![LeetCode](https://img.shields.io/badge/LeetCode_Knight-FFA116?style=for-the-badge&logo=leetcode&logoColor=black)](https://leetcode.com/inahus99)

![Profile Views](https://komarev.com/ghpvc/?username=inahus99&color=7c3aed&style=flat-square&label=Profile+Views&base=47)

</div>

---

## About Me

```ts
// warning: may spontaneously refactor your codebase at 2am

const suhani = {
  currentStatus:  "turning caffeine into APIs since 2024",
  superpower:     "making backends that don't fall over (usually)",
  knownBugs:      ["confuses 'quick fix' with 'rewrite from scratch'"],
  openTo:         "roles where prod DB isn't on someone's laptop",
  funFact:        "built a Redis clone instead of just using Redis"
};
```

---

## Work Experience

**Fencio.dev** · MTS Intern · *Jan 2026 – Present · Bengaluru*
- Replaced non-deterministic LLM scrapers causing bad DB writes; built a multi-agent system that auto-generates CSS selectors
- Cut external API costs by ~80% via LLM-driven profiling across 55+ feeds
- Reduced pagination latency by 66% through dynamic polling and DOM rendering bypass
- Built a threat-testing framework with data leakage, supply chain, and RBAC modules

**Adnoxy** · Software Developer Intern · *Nov 2025 – Jan 2026 · Gurugram*
- Set up hot/warm/cold storage tiers with Redis + Cloudflare R2, cutting egress costs to near-zero
- Built a schema validation engine for 19+ attributes with fuzzy schema mapping (50+ aliases → 14 canonical fields)
- Reduced external API calls by ~80% with a geo-intelligence caching layer powered by OpenStreetMap
- Automated PDF → structured JSON ingestion via LLMs (Gemini/Ollama), cutting onboarding effort by 3x

**NavaJuni News** · Software Developer Intern (Part-time) · *Oct 2025 – Jan 2026 · Gandhinagar*
- Designed backend from scratch: 20+ table relational schema and 30+ REST APIs
- Built content pipeline handling 100+ daily articles across 20+ RSS feeds with zero manual intervention
- Shipped a 3-tier admin panel (Super Admin / Admin / Moderator) for feed moderation and onboarding

**ShareWheelz LLP** · Node.js Developer Intern · *Jul 2025 – Sep 2025 · Remote*
- Built core backend with RESTful APIs (Express.js), OAuth-based auth, and optimized PostgreSQL schemas
- Integrated Socket.io for real-time communication in matched rider flows

---

## Featured Projects

| Project | Description | Tech | Links |
|---------|-------------|------|-------|
| **Third-Eye** | Real-time uptime monitoring with live WebSocket telemetry, SSL tracking, and Chart.js dashboards at 10ms latency | React · Socket.io · Express · MongoDB · Chart.js | [Live](https://third-eye-mu.vercel.app) · [GitHub](https://github.com/inahus99/ThirdEye) |
| **ComplyScan** | GDPR/CCPA compliance scanner — headlessly audits sites, scores 0–100, streams logs live via WebSockets | React · Express · Puppeteer · Socket.io · Mantine UI | [Live](https://complyscan.netlify.app) · [GitHub](https://github.com/inahus99/ComplyScan) |
| **ClauseWise** | Upload a legal PDF, get AI-powered clause extraction, risk flags, and plain-English summaries | React · Tailwind CSS · Multer · Gemini API | [Live](https://clause-wise.vercel.app) · [GitHub](https://github.com/inahus99/clauseWise) |
| **Curations** | Personal vault for notes, links, images & snippets — offline-capable, installable as a PWA | React · Supabase · Markdown · JavaScript | [Live](https://curations-vert.vercel.app) · [GitHub](https://github.com/inahus99/Curations) |
| **CRDT Editor** | Collaborative rich-text editor using CRDTs for zero-conflict multi-user sync over WebSockets | React · Node.js · WebSockets · Yjs | [GitHub](https://github.com/inahus99/CRDT-collaborative-editor) |
| **RediCore KV Store** | Redis-inspired key-value store built from scratch over TCP using the RESP protocol, with AOF persistence & TTL | Node.js · TCP · RESP · AOF | [GitHub](https://github.com/inahus99/RediCore) |

---

## Skills & Technologies

**Core**
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

**UI & Styling**
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-0055FF?style=flat-square&logo=framer&logoColor=white)
![Chakra UI](https://img.shields.io/badge/Chakra_UI-319795?style=flat-square&logo=chakraui&logoColor=white)
![Mantine](https://img.shields.io/badge/Mantine-339AF0?style=flat-square&logo=mantine&logoColor=white)

**Backend & Real-time**
![Socket.io](https://img.shields.io/badge/Socket.io-010101?style=flat-square&logo=socketdotio&logoColor=white)
![WebSockets](https://img.shields.io/badge/WebSockets-4353FF?style=flat-square&logo=websocket&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST_APIs-FF6C37?style=flat-square&logo=postman&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)

**Database & Storage**
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![Cloudflare R2](https://img.shields.io/badge/Cloudflare_R2-F38020?style=flat-square&logo=cloudflare&logoColor=white)

**AI & Automation**
![Gemini API](https://img.shields.io/badge/Gemini_API-4285F4?style=flat-square&logo=google&logoColor=white)
![Puppeteer](https://img.shields.io/badge/Puppeteer-40B5A4?style=flat-square&logo=puppeteer&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat-square&logo=n8n&logoColor=white)
![Playwright](https://img.shields.io/badge/Playwright-2EAD33?style=flat-square&logo=playwright&logoColor=white)

**Security & Auth**
![OAuth](https://img.shields.io/badge/OAuth_2.0-EB5424?style=flat-square&logo=auth0&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=flat-square&logo=jsonwebtokens&logoColor=white)
![Helmet.js](https://img.shields.io/badge/Helmet.js-05122A?style=flat-square&logo=nodedotjs&logoColor=white)

**DevOps & Tools**
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white)
![Netlify](https://img.shields.io/badge/Netlify-00C7B7?style=flat-square&logo=netlify&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=flat-square&logo=vite&logoColor=white)

---

## GitHub Stats

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=inahus99&theme=tokyonight&hide_border=true&background=0d1117&ring=7c3aed&fire=7c3aed&currStreakLabel=7c3aed)](https://git.io/streak-stats)

</div>

<div align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=inahus99&theme=tokyonight" height="160" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=inahus99&theme=tokyonight" height="160" />

</div>

---

## Contribution Graph

<div align="center">

[![Suhani's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=inahus99&bg_color=0d1117&color=7c3aed&line=7c3aed&point=c9d1d9&area=true&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

</div>

---

<div align="center">

*"Ship code that actually works."*

**Open to roles · [LinkedIn](https://linkedin.com/in/inahus99) · [Portfolio](https://inahus-portfolio.netlify.app/)**

</div>
